...
论文 Figure 3 给出了一个可以直接计算的结论：为什么 blocking 会让作弊不再有利可图。

设 A/B/C 三个文件大小都为 1，总缓存大小为 2，访问速率为：
- User 1：`A=10`，`B=5`（req/s）。
- User 2：`A=10`，`C=5`（req/s）。

在诚实的 max-min 分配下：
- `A` 被共享缓存；每个用户再缓存自己私有文件（`B` 或 `C`）的一半。
- 命中率按 `访问速率 × 缓存比例` 计算。
- 因此两人命中率都为：
...
现在 User 2 通过虚增 `C` 的访问来作弊，使 `C` 的优先级超过 `A`。
- 分配会变为：`A` 代表 User 1 缓存，`C` 代表 User 2 缓存。
- 在没有 blocking 的 max-min 中，User 2 仍可搭便车访问 `A`，于是：
...
在 FairRide 中，blocking 的触发条件是：**用户 `i` 访问了内存中已缓存的文件 `j`，但 `i` 不是该文件的缓存付费者/拥有者**。  
记 `n_j` 为“缓存 `j` 的其他用户数”，则：
...
结论是：在 FairRide 下，作弊不再获利。

:::remark 问题：不作弊时也会触发 blocking 吗？会不会降低命中？
会。论文明确指出，系统无法总是区分“策略性作弊”和“合法的访问模式变化”，因此在某些共享文件场景中，守规矩用户也可能被部分阻断。这正是 FairRide 牺牲 Pareto efficiency 的来源之一。但 FairRide 仍满足 isolation-guarantee：用户效用不低于静态隔离。
:::

:::remark 问题：用户能否通过反复重试绕过概率 blocking？
纯概率阻断在实现层面可能被重试探测削弱。FairRide 在系统实现中用 expected delaying 逼近概率阻断的期望效果，使重复探测在期望意义上不再有利，从而维持策略无关性。
:::
