...
它表示用户对未付费文件 `j` 的请求被**允许通过**的概率。

若用户在文件 `j` 上注入虚假流量 `r^{fake}_j`，则：
- 期望通过的虚假流量是 `q_j r^{fake}_j`；
- 期望被阻断的虚假流量是 `p(n_j) r^{fake}_j`。

结合课上缓存示例做一个可计算说明：
- 文件 A/B/C 各 1 GB，总缓存 2 GB。
- Bob 的真实需求是 `B:10 req/s, C:5 req/s`。
- 诚实状态下可看作缓存 `B=1`, `C=0.5`，于是 Bob 的真实命中率为

$$
HR_B^{honest}=\frac{10+0.5\times 5}{10+5}=\frac{12.5}{15}=83.3\%
$$

- Bob 通过注入虚假请求作弊：`A:+10 req/s`。
- 因为 Alice 也缓存 A，故 `n_A=1`，于是 `p(1)=50%`，即图中的 **Allow 5, Block 5**。

若被操纵后的需求让缓存从 C 偏向 A，Bob 的真实效用会下降：
- 一个与课件数值一致的代表性情形是 `C:0.5 -> 0`，则

$$
HR_B^{cheat}=\frac{10+0\times 5}{15}=\frac{10}{15}=66.7\%
$$

即便是更温和的偏移（`C:0.5 -> 0.25`），也有

$$
HR_B^{cheat}=\frac{10+0.25\times 5}{15}=\frac{11.25}{15}=75\%<83.3\%
$$

所以作弊不会产生“有用收益”：虚假流量一部分会被阻断，而缓存偏移会伤害 Bob 自己真正关心的对象。

:::remark 问题：不作弊时 blocking 也会发生吗？会不会照样降命中？
会。只要用户在共享文件上获取了“未付费的额外收益”，blocking 就可能触发，即使用户并非故意作弊。设计重点是先保护每个用户的基线公平份额，blocking 主要压制的是超额收益。因此可能损失一部分“额外命中”，但不应低于 isolation/share-guarantee 基线。作弊在此基础上还有额外副作用：更多被阻断/延迟的请求、被污染的需求信号，以及对自己真实热点对象的挤出效应。
:::
